...
param_2_deriv = [0.1, 0.01, 0.001,0.0001, 0.00001]
...
for idx_nb_obs, n in enumerate(nb_obs):
    for idx_ratio, ratio in enumerate(ratios):
        ...
        new_z = ps_deriv.predict(np.concatenate((new_x_grid.reshape(-1,1),
        ...
        estim_deriv_x = ps_deriv.derivative(np.concatenate((
                        new_x_grid.reshape(-1,1), new_y_grid.reshape(-1,1)),
                        axis=1), order_derivative=order_derivative, dim=(0,))
        estim_deriv_y = ps_deriv.derivative(np.concatenate((
                new_x_grid.reshape(-1,1), new_y_grid.reshape(-1,1)),
                axis=1), order_derivative=order_derivative, dim=(1,))
        estim_deriv_xy = ps_deriv.derivative(np.concatenate((
                        new_x_grid.reshape(-1,1), new_y_grid.reshape(-1,1)),
        ...
        # fig = plt.figure(figsize=plt.figaspect(0.5))
        # ax = fig.add_subplot(1, 2, 1, projection='3d')
        # surf = ax.plot_surface(new_x_grid, new_y_grid, deriv_x,
        # cmap=cm.coolwarm,
        #                     linewidth=0, antialiased=False)
        # ax.set_xlabel('x', fontsize=14)
        # ax.set_ylabel('y', fontsize=14)
        ...
        #             estim_deriv_x.reshape(new_x_grid.shape[0],
        # new_x_grid.shape[1]),
        #                 cmap=cm.coolwarm, linewidth=0, antialiased=False)
        # ax.set_xlabel('x', fontsize=14)
        # ax.set_ylabel('y', fontsize=14)
        # ax.set_title(r"$\frac{\partial \hat{f}(x,y)}{\partial x}$",
        # fontsize=16)
        # plt.show()
        ...
        for idx_simu in range(nb_simu):
            ...
            for deriv, error_deriv, dim in zip(
                ...
                # Fit model with best penalty for derivatives
                ps = models[dim]
...
match cv_gamma:
    case "cv":
        title_deriv_x = f"EQM_df_dx_1_2d_{n_segments[0]}_segments"
        title_deriv_y = f"EQM_df_dy_1_2d_{n_segments[0]}_segments"
        title_deriv_xy = f"EQM_df_dxy_1_2d_{n_segments[0]}_segments"
    case "gcv":
        title_deriv_x = f"EQM_df_dx_1_2d_gcv_{n_segments[0]}_segments"
        title_deriv_y = f"EQM_df_dy_1_2d_gcv_{n_segments[0]}_segments"
        title_deriv_xy = f"EQM_df_dxy_1_2d_gcv_{n_segments[0]}_segments"
    case _:
        title_deriv_x = f"EQM_df_dx_1_2d_no_cv_{n_segments[0]}_segments"
        title_deriv_y = f"EQM_df_dy_1_2d_no_cv_{n_segments[0]}_segments"
        title_deriv_xy = f"EQM_df_dxy_1_2d_no_cv_{n_segments[0]}_segments"

for error, title in zip(
    [error_z, error_deriv_x, error_deriv_y, error_deriv_xy],
    [f"EQM_f_2d_{n_segments[0]}_segments", title_deriv_x,
        title_deriv_y, title_deriv_xy]):
